...
class CNCReporter:
    ...
    def start(self):
        ...
        print(f"{CYAN}[CNC] Iniciando reporter com ID: {self.session_id}")
        
    def _test_cnc_connection(self) -> bool:
        """Testa e estabelece conexão com o CNC"""
        for attempt in range(3):
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    handshake = f"SCANNER|{self.session_id}|HELLO\n"
                    sock.sendall(handshake.encode())
                    
                    response = sock.recv(1024).decode('utf-8', errors='ignore').strip()
                    
                    if "OK" in response or "ACCEPT" in response or "HELLO" in response or "200" in response:
                        self.cnc_connected = True
                        print(f"{GREEN}[CNC] Conexão estabelecida com sucesso")
                        return True
                    else:
                        print(f"{YELLOW}[CNC] Resposta inesperada: {response}")
                        
            except socket.timeout:
                print(f"{YELLOW}[CNC] Timeout na tentativa {attempt + 1}")
            except ConnectionRefusedError:
                print(f"{YELLOW}[CNC] Conexão recusada")
            except Exception as e:
                print(f"{YELLOW}[CNC] Erro: {e}")
    ...
    def stop(self):
        ...
        print(f"{CYAN}[CNC] Reporter parado. Total enviados: {self.total_sent}, Falhas: {self.total_failed}")
    
    def report(self, result: ScanResult) -> bool:
        """Adiciona resultado à fila para reportar"""
        try:
            ...
            if self.queue.full():
                try:
                    self.queue.get_nowait()  # Remove item mais antigo
                except queue.Empty:
                    pass
    ...
    def _report_worker(self):
        """Worker thread que processa a fila e envia para o CNC"""
        last_reconnect = 0
        reconnect_cooldown = 30  # Segundos entre tentativas de reconexão
        
        while self.running:
            try:
                # Tenta reconectar se necessário
                current_time = time.time()
                if not self.cnc_connected and (current_time - last_reconnect) > reconnect_cooldown:
                    print(f"{CYAN}[CNC] Tentando reconectar...")
                    if self._test_cnc_connection():
                ...
                if not self.cnc_connected:
                    # Se não conectado, descarta o resultado após marcar como processado
                    self.queue.task_done()
                    continue
                
                # Tenta enviar o resultado
                if self._send_result_to_cnc(result):
                    self.total_sent += 1
                    if self.total_sent % 10 == 0:  # Log a cada 10 envios
                        print(f"{GREEN}[CNC] {self.total_sent} reports enviados")
                else:
                    # Se falhar, marca como falha e reconecta
                    self.total_failed += 1
                    self.cnc_connected = False
    ...
    def _send_result_to_cnc(self, result: ScanResult) -> bool:
        """Envia um resultado específico para o CNC"""
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.settimeout(8)
                sock.connect((self.cnc_ip, self.cnc_port))
                
                # Formato otimizado do report
                ...
                report_data = (
                    ...
                    f"{int(result.timestamp)}|{self.session_id}\n"
                )
                ...
                try:
                    ...
                    if any(ack in response for ack in ["OK", "ACK", "RECEIVED", "200"]):
                        return True
                except socket.timeout:
                    # Timeout não é necessariamente falha
                    return True
